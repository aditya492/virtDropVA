...
class ThirdEnquiryPage extends Component {
  ...
  render() {
    const list = [
      { tab: "Su", bool: this.state.satColor, full: "Sunday" },
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <div>
                        <form onChange={this.submitForm} className="timeForm">
    ...
    for (var i = 0; i < time.length; i++) {
      ...
      time[i].addEventListener("keyup", function (e) {
        ...
        if (this.value.length == 2 && reg.test(this.value))
          this.value = this.value + ":"; //Add colon if string length > 2 and string is a number
        if (this.value.length > 5)
          this.value = this.value.substr(0, this.value.length - 1); //Delete the last digit if string length > 5
      });
    }
  ...
  handleDays = (id, tab, increment, index, fullName) => {
    ...
    demodaysback.forEach((element) => {
      if (element.hasOwnProperty(increment)) {
        if (element[increment].includes(tab)) {
          toast.error(`Removed ${fullName} For ${increment} VA`);
    ...
    this.setState(
      { setColor: demodaysback[Object.keys(demodaysback)[index]]?.[increment] },
      () => {}
    );
